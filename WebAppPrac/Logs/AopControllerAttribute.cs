...
namespace WebAppPrac.Logs
{
    public class AopControllerAttribute : ActionFilterAttribute
    {
        ...
        {
            StaticLogger.Information($"Result from {method}: {result.StatusCode} | Value: {GetValue(result.Value)}");
        }

        private void LogEvent(string method, string result)
        {
            StaticLogger.Information($"Result from {method}: {result}");
        }
        ...
        private string GetValue(object toPrint)
        {
            ...
            {
                StaticLogger.Error($"The result object can not be represented. Please use serializable objects: {ex.Message} | {ex.InnerException} ");
            }
